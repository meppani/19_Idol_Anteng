...
import { Search, Sparkles, Music2, Lock } from "lucide-react";

export const ParticipantGrid: React.FC = () => {
  const [selectedCategory, setSelectedCategory] = useState<string>("Semua");
  const [searchQuery, setSearchQuery] = useState<string>("");
  const [showLockToast, setShowLockToast] = useState<boolean>(false);

  const categories = [
    { id: "Semua", label: "Semua", locked: false },
    { id: "Finalist", label: "Finalist", locked: true },
  ];

  const handleCategoryClick = (cat: { id: string; label: string; locked: boolean }) => {
    if (cat.locked) {
  ...
  return (
    <section id="participants" className="relative py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto z-10">
      ...
      {showLockToast && (
        <div className="mb-6 mx-auto max-w-md p-3.5 rounded-xl bg-slate-900/90 border border-amber-500/50 text-amber-300 text-xs sm:text-sm font-bold text-center flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(212,175,55,0.3)] animate-bounce">
          <Lock className="w-4 h-4 text-amber-400" />
          <span>Kategori Finalist Belum Dibuka! Tahap Kualifikasi Masih Berlangsung.</span>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-10 p-4 rounded-2xl glass-panel border border-cyan-500/20">
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            type="text"
            placeholder="Cari nama, RT, atau lagu..."
            value={searchQuery}
