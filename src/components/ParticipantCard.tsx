...
export const ParticipantCard: React.FC<ParticipantCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className="group relative cursor-pointer rounded-2xl overflow-hidden glass-panel border border-cyan-500/20 hover:border-cyan-400/80 transition-all duration-300 shadow-card-glow hover:shadow-hover-card flex flex-col"
    >
      ...
      <div className="relative w-full h-72 sm:h-80 overflow-hidden bg-slate-950">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 bg-slate-950/40 backdrop-blur-[2px]">
          <motion.div
            ...
            className="w-16 h-16 rounded-full bg-gradient-to-tr from-cyan-500 via-sky-300 to-blue-600 p-0.5 shadow-[0_0_30px_rgba(0,240,255,0.8)]"
          >
      ...
      <div className="p-5 flex-1 flex flex-col justify-between relative z-10 bg-[#0b1129]/90 backdrop-blur-md">
        <div>
          ...
          {/* Full Name */}
          <h3 className="text-xl font-black text-white group-hover:text-cyan-300 transition-colors duration-200 tracking-wide">
            {participant.name}
          </h3>

          {/* Song Performance Title */}
          <div className="mt-2.5 flex items-start gap-2 p-2.5 rounded-xl bg-slate-900/80 border border-slate-800 group-hover:border-cyan-500/30 transition-colors">
            <Music className="w-4 h-4 text-cyan-400 shrink-0 mt-0.5" />
